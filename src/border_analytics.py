...
def read_file_store_into_dictionary(input_file_location):
    ...
    try:
        with open(input_file_location) as _file:
            ...
            header_actual = ['Port Name','State','Port Code','Border','Date','Measure','Value','Location']
            ...
            for _line in _file:
                ...
                try: int(words[6]) ## Check if any NA values are found in the "Value" field
...
def data_process_get_average(input_data_dictionary):
    # define a new array 
    processed_data = []
    ...
    for border_key in input_data_dictionary.keys():
        for measure_key in input_data_dictionary[border_key]:
            ...
            value_months = {}
            for date_key1 in input_data_dictionary[border_key][measure_key]:
                try:
                    yr = date_key1.split(" ")[0].split("/")[2]
                    mn = date_key1.split(" ")[0].split("/")[0]
                    if yr in value_months.keys():
                        value_months[yr][int(mn)-1]= int(input_data_dictionary[border_key][measure_key][date_key1])
                    else:
            ...
            for date_key2 in input_data_dictionary[border_key][measure_key]:
                yr = date_key2.split(" ")[0].split("/")[2]
                mn = int(date_key2.split(" ")[0].split("/")[0])
                if mn == 1:
                    processed_data.append(data_point(border_key,date_key2,measure_key,
                                                  (input_data_dictionary[border_key][measure_key][date_key2]),
                                                  0,yr,mn))
                elif (mn == 2):
                    processed_data.append(data_point(border_key,date_key2,measure_key, 
                                                  (input_data_dictionary[border_key][measure_key][date_key2]),
...
input_data_dictionary = read_file_store_into_dictionary(input_file_location)


## Process the data, get moving average, and insert the data into a multidimentional array
processed_data = data_process_get_average(input_data_dictionary)
...
try:
    ...
    for row in processed_data_ordered:
        output_file.write(str(row.border)+","+str(row.date)+","+str(row.measure)+","+str(row.value)+","+str(row.average)+"\n")
    output_file.close()
except:
